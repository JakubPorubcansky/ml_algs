...
class LogisticRegressionMiniBatchGradientDescent:
    ...
    def fit(self, X, y, batch_size: float | int = 0.2) -> 'LogisticRegressionMiniBatchGradientDescent':
    ...
    def fit_on_batch(self, X, y):
        ...
        self._weights += self.learning_rate * (errors.dot(X) / X.shape[0])
        self._bias += self.learning_rate * errors.mean()

    def _calculate_loss(self, X, y) -> float:
        pred = self.activation(self.net_input(X))
        return (-y.dot(np.log(pred)) - (1 - y).dot(np.log(1 - pred))) / X.shape[0]
